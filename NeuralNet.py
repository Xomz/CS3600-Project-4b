...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        """YOUR CODE"""
        inActs.insert(0, 1)
        out = self.sigmoid(self.getWeightedSum(inActs))
        inActs.pop(0)
        return out
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """YOUR CODE"""
        inActs.insert(0, 1)
        out = self.sigmoidDeriv(self.getWeightedSum(inActs))
        inActs.pop(0)
        return out
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for weight in self.weights:
        	weightUpdate = weight + alpha * inActs[index] * delta
        	index += 1
        	newWeights.append(weightUpdate)
        	totalModification += abs(weight - weightUpdate)
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
        	thisLayerOut = []
        	for node in layer:
        		thisLayerOut.append(node.sigmoidActivation(thisInput))
        	output.append(thisLayerOut)
        	thisInput = thisLayerOut
        return output
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for test in examplesTest:
    	inp = test[0]
    	testOut = test[1]
    	outputs = nnet.feedForward(inp)[-1]
    	isEqual = True
    	for i in range(0, len(outputs)):
    		if round(outputs[i]) != testOut[i]:
    			isEqual = False
    	if isEqual:
    		testCorrect += 1.0
    	else:
